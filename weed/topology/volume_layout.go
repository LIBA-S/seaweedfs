...
func NewVolumeBinaryStatus(name volumeState, copyCount int, indicator stateIndicator) *volumesBinaryState {
...
func NewVolumeLayout(rp *super_block.ReplicaPlacement, ttl *needle.TTL, volumeSizeLimit uint64, replicationAsMin bool) *VolumeLayout {
	return &VolumeLayout{
		...
		readonlyVolumes:  NewVolumeBinaryStatus(readOnlyState, rp.GetCopyCount(), ExistCopies()),
		oversizedVolumes: NewVolumeBinaryStatus(oversizedState, rp.GetCopyCount(), ExistCopies()),
